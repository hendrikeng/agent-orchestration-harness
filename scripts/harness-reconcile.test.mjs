...
for (const legacy of [false, true]) test(`reconciliation records reviewed files without bypassing guards (legacy=${legacy})`, async (t) => {
  ...
  await fs.writeFile(path.join(target, 'README.md'), custom);
  ...
  const readme = (value) => value.files.find((file) => file.targetPath === 'README.md');
  const changes = [
  ...
  for (const change of changes) {
    ...
    assert.equal(await fs.readFile(path.join(target, 'README.md'), 'utf8'), custom);
  }
  await fs.writeFile(path.join(target, reviewRel), JSON.stringify(review));
  await fs.writeFile(path.join(target, 'README.md'), 'Changed after approval.\n');
  assert.equal(run(target, 'reconcile', '--review', reviewRel).status, 1);
  assert.equal(await fs.readFile(manifestPath, 'utf8'), before);
  await fs.rename(path.join(target, 'README.md'), path.join(target, 'readme-backup.txt'));
  assert.equal(run(target, 'reconcile', '--review', reviewRel).status, 1);
  await fs.symlink('readme-backup.txt', path.join(target, 'README.md'));
  ...
  assert.equal(after.managedFiles.find((entry) => entry.targetPath === 'README.md').preservedLocal, true);
